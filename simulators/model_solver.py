...
class SoftFireCA(nn.Module):
    ...
    def forward(self, ignition_point, wind_grid, n_steps):
        """
        Runs the soft CA for n_steps, mirroring the true CA loop.
        Returns predicted_arrival : (H, W)  ∈ [0, n_steps]

        ignition_point : (i0, j0)
        wind_grid      : (H, W) array or tensor — wind specific to this fire
        n_steps        : number of CA steps

        Soft arrival time:
            E[arrival[i,j]] +-= sum_t  t · P(first crossing at t)
                             +-= sum_t  t · clamp(state_t _ state_{t-1}, 0)
        """
        i0, j0 = ignition_point
        state = torch.zeros(self.H, self.W)
        state[i0, j0] = 0.8        # initial ignition (matches true CA)
        ...
        for t in range(1, n_steps + 1):
            ...
            # Increment in burn probability +-= probability of first ignition at t
...
def fit(model, fires_data, n_steps=100, n_epochs=150, lr=0.05, verbose=True):
    """
